...
class OpenRouterService(
        ...
        private val logRepository: com.murmli.webpursuer.data.LogRepository? = null
) {
        ...
        suspend fun checkContent(
                ...
                val client =
                        OkHttpClient.Builder()
                                ...
                                .build()
                // ... (rest of client setup same as before, likely could refactor but keeping
                // changes
                // minimal for now)
                val retrofit =
                        Retrofit.Builder()
        ...
        suspend fun interpretContent(
                ...
                try {
                        ...
                        val result =
                                response.choices.firstOrNull()?.message?.content?.trim() ?: content
                        logRepository?.logInfo(
                                "LLM",
                                "interpretContent success, length: ${result.length}"
                        )
                        return result
                } catch (e: Exception) {
                        e.printStackTrace()
